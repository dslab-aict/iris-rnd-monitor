...
6. 상세 화면의 `링크공유` 팝업 또는 HTML의 `ancmId`, `ancmPrg` 값을 읽어 `https://www.iris.go.kr/contents/retrieveBsnsAncmView.do?ancmId=...&ancmPrg=...` 형식의 공유 링크를 저장합니다.
7. `공고번호 + 공고명 + 공고일자` 해시로 중복 append를 방지합니다.
...
- `debug_iris_share_p*_i*.html`: 링크공유 팝업 클릭 후 HTML
